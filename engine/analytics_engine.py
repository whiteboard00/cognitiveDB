...
Handles performance snapshots, cognitive efficiency calculations,
and comprehensive learning analytics for the cognition business.
"""
...
class AnalyticsEngine:
    """Engine for comprehensive learning analytics"""
    ...
    async def _calculate_cognitive_metrics(self, user_id: str) -> Dict[str, Optional[float]]:
        """
    ...
    async def _get_recent_performance_stats(self, user_id: str) -> Dict[str, int]:
        """
    ...
    async def _get_recent_practice_data(self, user_id: str) -> List[Dict[str, Any]]:
        """
        Get recent practice data for efficiency calculations.

        Args:
            user_id: Student ID

        Returns:
            List of recent practice sessions with metadata
        """
        cutoff_date = datetime.now() - timedelta(days=30)

        try:
            # Get recent answered questions with all metadata
            response = self.db.table("session_questions").select(
                "time_spent_seconds, confidence_score, user_answer, "
                "questions(correct_answer, difficulty), answered_at"
            ).eq("status", "answered").gte(
                "answered_at", cutoff_date.isoformat()
            ).in_(
                "session_id",
            ...
            print(f"Error getting recent practice data: {e}")
            return []

    async def get_skill_heatmap(self, user_id: str) -> Dict[str, Any]:
        """
